...
async function sendModifyStatus(modifyList) {
    try {
        ...
        for (const key of modifyList) {
            if (key.dateTo == today) {
                const result = await axios.post(`http://172.16.0.2:3000/forward?exten=${key.exten}&type=${key.type}&number=${key.number}&dateFrom=${key.dateFrom}&dateTo=${key.dateTo}&status=false`);
                logger.info(`Результат изменений ${result}`);
                const resultDelete = await deleteIDInDB(key.id);
                logger.info(`Удалены данные ${resultDelete}`);
            }
        }
    } catch (e) {
